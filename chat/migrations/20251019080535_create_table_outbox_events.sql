...
COMMENT ON COLUMN outbox_events.aggregate_id     IS 'Идентификатор агрегата (например, message_id)';
COMMENT ON COLUMN outbox_events.aggregate_type   IS 'Тип агрегата (например, message_sent)';
COMMENT ON COLUMN outbox_events.event_type       IS 'Тип события (например, chat.message.sent)';
