...
func (r *Repository) SearchEvents1(ctx context.Context, opts ...appoutbox.SearchEventsOption) []*appoutbox.Event {
	query := r.sb.Select("id").From("friend_requests").
		Where(squirrel.Eq{"id": "e0bab788-eaab-4c86-9237-6e28a67572da"})

	conn := r.db.GetQueryEngine(ctx)

	log.Println(query.ToSql())

	var rows []outboxEvent1
	if err := conn.Selectx(ctx, &rows, query); err != nil {
